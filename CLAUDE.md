...
- **Blog** — Markdown files in `content/posts/` processed at build time by `lib/posts.ts`. The pipeline: js-yaml (frontmatter) → remark → rehype → sanitized HTML. `getRelatedPosts(slug, tags)` returns up to 3 posts sharing tags, used on post pages.
